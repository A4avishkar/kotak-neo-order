#!/usr/bin/env python3
"""
Fetch Live Data for All Nifty Option Strikes (No SDK Version)
Reads option tokens from niftycurrentexpirypsymbol.py output CSV
and fetches live market data for each strike price

This version uses direct REST API calls instead of the SDK.
Note: WebSocket functionality still requires HSWebSocketLib module for binary protocol handling.
"""
...
class AllStrikesDataFetcher:
    ...
    def get_nifty_cmp(self):
        """Get Nifty 50 Current Market Price (CMP) via WebSocket"""
        print("  📡 Fetching Nifty 50 CMP...")
        
        try:
            from neo_api_client.HSWebSocketLib import (
                prepareConnectionRequest2, prepareSubsUnSubsRequest,
                HSWrapper, BinRespTypes, INDEX_PREFIX
            )
        except ImportError:
            print("  ❌ Could not import WebSocket binary protocol helpers")
            return None
        ...
        def on_open(ws):
            ...
            ws.send(conn_bytes, opcode=0x2)
        
        def on_message(ws, message):
            nonlocal connection_ok, subscription_ok, nifty_cmp
        ...
        try:
            ...
            if not data_received.wait(timeout=10):
                print("  ⚠️  Timeout waiting for Nifty CMP")
    ...
    def get_quotes_via_websocket(self, tokens, exchange_segment="nse_fo", timeout_seconds=30, persistent=False):
        ...
        try:
            from neo_api_client.HSWebSocketLib import (
                prepareConnectionRequest2, prepareSubsUnSubsRequest,
                HSWrapper, BinRespTypes, SCRIP_PREFIX
            )
        except ImportError:
            print("  ❌ Could not import WebSocket binary protocol helpers")
            print("     This requires the SDK's HSWebSocketLib module")
    ...
    def open_persistent_websocket(self, tokens, batch_quotes, shared_quotes_dict):
                    ...
                    if isinstance(parsed, list) and len(parsed) > 0:
                        ...
                        if isinstance(item, dict):
                            if item.get('type') == 'cn' and item.get('stat') == 'Ok':
                                ...
                                # Subscribe to tokens
                                scrips = "&".join([f"nse_fo|{token}" for token in tokens])
                                sub_bytes = prepareSubsUnSubsRequest(
                                    scrips,
                                    BinRespTypes["SUBSCRIBE_TYPE"],
                                    SCRIP_PREFIX,
    ...
    def find_latest_output_csv(self, output_dir='outputs'):
        """Find the latest output CSV file from niftycurrentexpirypsymbol.py"""
        today_str = datetime.now().strftime('%Y%m%d')
        today_pattern = os.path.join(output_dir, f"nifty_nearest_expiry_{today_str}_*.csv")
        candidates = glob.glob(today_pattern)
        
        if not candidates:
            # Fallback to any file
            pattern = os.path.join(output_dir, "nifty_nearest_expiry_*.csv")
    ...
    def fetch_all_strikes_data(self, input_csv=None, batch_size=10, fetch_interval=None, oi_calc_interval=None, continuous_mode=False):
        ...
        print("="*80)
        print("📊 FETCH ALL NIFTY STRIKES LIVE DATA")
        print("="*80)
        ...
        if not input_csv:
            ...
            if not input_csv:
                ...
                print("   Please run niftycurrentexpirypsymbol.py first or specify CSV path")
                return
        ...
        print(f"\n📡 Step 1: Getting Nifty CMP (Current Market Price)...")
        
        # First, get Nifty 50 spot price
        nifty_cmp = self.get_nifty_cmp()
        if not nifty_cmp:
            print("❌ Could not get Nifty CMP. Cannot filter strikes.")
            return
        
        print(f"\n📊 Nifty CMP: ₹{nifty_cmp:.2f}")
        print(f"\n📡 Step 2: Filtering strikes (40 above + 40 below CMP)...")
        
        # Filter tokens based on strike price
        # Get 40 strikes above and 40 strikes below CMP for both CE and PE
        filtered_tokens_data = self.filter_strikes_around_cmp(tokens_data, nifty_cmp, num_strikes=40)
        ...
        for batch_idx in range(0, len(all_tokens), max_batch_size):
            ...
            quotes = self.get_quotes_via_websocket(batch_tokens, timeout_seconds=timeout_sec)
        ...
        if not all_quotes_data:
            ...
            all_quotes_data = self.get_quotes_via_rest_batch(all_tokens, exchange_segment="nse_fo")
            if not all_quotes_data:
        ...
        output_data.append(nifty_cmp_row)
        ...
        # Calculate total OI for Call and Put sides
        total_oi_ce = 0
        total_oi_pe = 0
        ...
        cmp_str = f"{nifty_cmp:.0f}" if nifty_cmp else 'UNKNOWN'
        output_filename = f"nifty_strikes_data_{today_str}_{expiry_str}_CMP{cmp_str}.csv"
        ...
        print(f"   Total rows: {len(output_data)} (including 1 Nifty row + 2 summary rows)")
        print(f"   Tokens with data: {len(all_quotes_data)}")
        print(f"   Tokens with N/A: {len(filtered_tokens_data) - len(all_quotes_data)}")
        print(f"   Nifty CMP: ₹{nifty_cmp:.2f} (added to CSV)")
        print(f"   Strikes filtered: 40 above + 40 below CMP")
        ...
        pattern1 = os.path.join(output_dir, f"nifty_all_strikes_data_*.csv")
        pattern2 = os.path.join(output_dir, f"nifty_strikes_data_*.csv")
        ...
        for existing_path in candidates:
            ...
            if (existing_name.startswith(f"nifty_all_strikes_data_{today_str}_") or 
                existing_name.startswith(f"nifty_strikes_data_{today_str}_")):
    ...
    def fetch_continuous_live_data(self, input_csv=None, fetch_interval=None, oi_calc_interval=None):
        ...
        # Get Nifty CMP and filter strikes
        print(f"\n📡 Getting Nifty CMP...")
        nifty_cmp = self.get_nifty_cmp()
        if not nifty_cmp:
            print("❌ Could not get Nifty CMP")
            return
        ...
        today_str = datetime.now().strftime('%Y%m%d')
        expiry_str = filtered_tokens_data[0]['expiry'] if filtered_tokens_data else 'UNKNOWN'
        cmp_str = f"{nifty_cmp:.0f}"
        output_filename = f"nifty_strikes_live_{today_str}_{expiry_str}_CMP{cmp_str}.csv"
        output_path = os.path.join(output_dir, output_filename)
        ...
        for batch_idx in range(0, len(all_tokens), max_batch_size):
            ...
            ws_conn = self.open_persistent_websocket(batch_tokens, batch_quotes, all_quotes_data)
            if ws_conn:
        ...
        try:
            while True:
                ...
                # Get fresh Nifty CMP for this iteration
                print(f"\n📡 Getting Nifty CMP for this iteration...")
                current_nifty_cmp = self.get_nifty_cmp()
                if not current_nifty_cmp:
                    current_nifty_cmp = nifty_cmp  # Fallback to initial CMP
                    print(f"   ⚠️  Using previous CMP: ₹{current_nifty_cmp:.2f}")
                else:
                    print(f"   ✓ Current CMP: ₹{current_nifty_cmp:.2f}")
                ...
                # Add Nifty CMP as first row (updated with current CMP)
                nifty_cmp_row = {
                    'token': 'NIFTY_50',
                    'pSymbol': 'NIFTY 50',
                    'pOptionType': 'INDEX',
                ...
                for token_info in filtered_tokens_data:
                    ...
                    # Calculate OI totals
                    try:
                        if oi != 'N/A' and oi is not None:
                            oi_float = float(oi)
                            if token_info['option_type'] == 'CE':
                                total_oi_ce += oi_float
                            elif token_info['option_type'] == 'PE':
                                total_oi_pe += oi_float
                    except (ValueError, TypeError):
                        pass
                ...
                try:
                    ...
                    print(f"   Total rows: {len(output_data)} (including 1 Nifty row)")
                    print(f"   Nifty CMP: ₹{current_nifty_cmp:.2f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Fetch live data for all Nifty option strikes')
