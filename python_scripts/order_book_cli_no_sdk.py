#!/usr/bin/env python3
"""
Direct REST API Order Book CLI (No SDK).

Fetches the full order book via:

    GET quick/user/orders

Authentication reuses the direct TOTP login + MPIN validate flow
from the other *_cli_no_sdk scripts and reads credentials from b.txt.
"""
...
def fetch_order_book(base_url: str, edit_token: str, edit_sid: str, server_id: str) -> Dict[str, Any]:
    """GET order book entries."""
    url = f"{base_url.rstrip('/')}/{ORDER_BOOK_ENDPOINT}"
    headers = {
        "Sid": edit_sid,
        "Auth": edit_token,
...
def main():
    parser = argparse.ArgumentParser(description="Direct REST API Order Book CLI (No SDK)")
