...
# Filter for NIFTY rows
nifty_rows = df[df['pSymbolName'] == 'NIFTY']
print(f"📊 Found {len(nifty_rows)} NIFTY rows in CSV")

if len(nifty_rows) == 0:
    print("❌ Error: No NIFTY rows found in the CSV file")
    sys.exit(1)

# Select the 3 original columns
result = nifty_rows[['pSymbol', 'pOptionType', 'pScripRefKey']].copy()

# Function to extract strike price from pScripRefKey
def extract_strike_price(scrip_ref_key):
    # Pattern: NIFTY[DDMMMYY][STRIKE].00[CE/PE]
    # Examples: NIFTY04NOV2519350.00PE -> 19350 (not 2519350)
    #          NIFTY30DEC2528800.00PE -> 28800 (not 2528800)
    #          NIFTY11NOV2525000.00PE -> 25000 (not 2525000)
...
def extract_expiry(scrip_ref_key):
    ...
    # Examples: NIFTY30DEC2528800.00PE -> 30DEC25
    #          NIFTY25NOV2520550.00PE -> 25NOV25
    #          NIFTY11NOV2525000.00PE -> 11NOV25
    # The expiry comes after NIFTY and before the strike price
...
if len(result) == 0:
    print("❌ Error: Could not parse any valid expiry dates from NIFTY rows")
...
try:
    ...
    for existing_path in candidates:
        ...
        try:
            # Extract date from filename: nifty_expiry_{YYYYMMDD}_{expiry}.csv
            if existing_name.startswith(f"nifty_expiry_{today_str}_"):
        ...
        if file_mod_date == today_date or file_date_in_name == today_date:
            # Extract expiry from filename: nifty_expiry_{date}_{expiry}.csv
            try:
                parts = existing_name.replace('.csv', '').split('_')
                if len(parts) >= 4:  # nifty_expiry_date_expiry
...
print(f'\n📊 NIFTY rows for SELECTED EXPIRY: {selected_expiry_str}')
print('=' * 100)
