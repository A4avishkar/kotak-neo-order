#!/usr/bin/env python3
"""
Direct REST API Portfolio Holdings CLI (No SDK).

Uses the same TOTP login + MPIN validate flow as `place_order_cli_no_sdk.py`,
then calls the holdings endpoint:

    GET portfolio/v1/holdings

Reads credentials from `b.txt` in the project root:
- KOTAK_CONSUMER_KEY
- KOTAK_MOBILE_NUMBER
- KOTAK_MPIN
- KOTAK_UCC
- KOTAK_TOTP_SECRET
- (optional) KOTAK_NEO_FIN_KEY
"""
...
HOLDINGS_ENDPOINT = "portfolio/v1/holdings"
...
def fetch_holdings(base_url: str, edit_token: str, edit_sid: str, server_id: str) -> Dict[str, Any]:
    """GET holdings from the portfolio endpoint."""
    url = f"{base_url.rstrip('/')}/{HOLDINGS_ENDPOINT}"
    headers = {
        "Sid": edit_sid,
        "Auth": edit_token,
        "accept": "*/*",
...
def main():
    parser = argparse.ArgumentParser(description="Direct REST API Portfolio Holdings CLI (No SDK)")
